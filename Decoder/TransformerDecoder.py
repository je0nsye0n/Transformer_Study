...
class TransformerEmbedding(nn.Module):
    def __init__(self, d_model, drop_prob, max_len, vocab_size, device):
        ...
        self.token_emb.weight.detach().cpu().numpy().astype(np.float32).tofile(os.path.join("./Data", "tok_emb.bin"))

    def forward(self, x):
        return self.token_emb(x) + self.positional_encoding[: x.size(1), :]
...
def save_input_data(trg, enc_src, trg_mask, src_mask, folder="Data"):
    os.makedirs(folder, exist_ok=True)

    trg.cpu().numpy().astype(np.int32).tofile(os.path.join(folder, "trg.bin"))
    enc_src.cpu().numpy().astype(np.float32).tofile(os.path.join(folder, "enc_src.bin"))
    trg_mask.cpu().numpy().astype(np.float32).tofile(os.path.join(folder, "trg_mask.bin"))
    src_mask.cpu().numpy().astype(np.float32).tofile(os.path.join(folder, "src_mask.bin"))

    print(f"Saved input data to {folder}/ as .bin files")


def save_linear_weights_biases(linears, name, folder="Data"):
    os.makedirs(folder, exist_ok=True)

    weight_file = os.path.join(folder, f"{name}_linear_weights.bin")
    bias_file = os.path.join(folder, f"{name}_linear_biases.bin")

    linears.weight.detach().cpu().numpy().astype(np.float32).tofile(weight_file)
    linears.bias.detach().cpu().numpy().astype(np.float32).tofile(bias_file)
...
save_input_data(trg,enc_src,trg_mask,src_mask)
